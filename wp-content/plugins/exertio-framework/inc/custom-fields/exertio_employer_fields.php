 ...
 *
 * @package    exertio-framework
 * @subpackage exertio-framework/admin
 * @author     ScriptsBundle
 */
class Exertio_Employer_Acf_Fields
{
    public function __construct()
    {
        add_action('departments_add_form_fields', array($this, 'exertio_employer_acf_field_term_meta_cat_template'), 10, 1);
        add_action('departments_edit_form_fields', array($this, 'exertio_employer_acf_field_term_meta_cat_template'), 10, 1);
        add_action('edit_departments', array($this, 'exertio_employer_acf_save_term_meta_cat_template'));
        add_action('create_departments', array($this, 'exertio_employer_acf_save_term_meta_cat_template'));
		
        add_filter('exertio_employer_acf_template_fields', array($this, 'exertio_employer_acf_template_fields'), 10, 1);
        add_filter('exertio_employer_backend_fields_html', array($this, 'exertio_employer_acf_backend_fields_html_callback'), 10, 3);
        add_filter('exertio_employer_acf_frontend_html', array($this, 'exertio_employer_acf_show_frontend_fields_callback'), 10, 5);
    }

    function exertio_employer_acf_show_frontend_fields_callback($html_data = '', $fields_data = array(), $post_id = 0, $col = 'col-6', $cus_type = 'default')
    {
        $html_data = '';
    ...
    function exertio_employer_acf_backend_fields_html_callback($html_data = '', $fields_data = array(), $post_id = 0)
    {
        ob_start();
        if (isset($fields_data) && is_array($fields_data) && sizeof($fields_data) > 0) {
            foreach ($fields_data as $field_key => $field_val) {
    ...
    function exertio_employer_acf_save_term_meta_cat_template($term_id)
    {
    ...
    function exertio_employer_acf_field_term_meta_cat_template($taxonomy)
    {
...
new Exertio_Employer_Acf_Fields();


add_action('acf/render_field_settings', 'exertio_employer_acf_render_field_settings');
function exertio_employer_acf_render_field_settings($field)
{
...
// to get group key
add_filter('exertio_employer_acf_get_group_key', 'exertio_employer_acf_get_template_key_callback', 10, 2);
if (!function_exists('exertio_employer_acf_get_template_key_callback')) {
    function exertio_employer_acf_get_template_key_callback($template_group_key = '', $term_id = '')
    {
        ...
        if ($template_group_key == '' && $cat_template_type == 'hierarchal') {
            ...
            $term_parents = get_ancestors($term_id, 'departments');
            ...
            if (isset($term_parents) && !empty($term_parents) && sizeof($term_parents) > 0) {
                ...
                if ($template_group_key == '') {
                    $template_group_key = apply_filters('exertio_employer_acf_get_group_key', '', $term_id);
                }
            }
        }
        return $template_group_key;
...
if (!function_exists('exertio_employer_fields_by_listing_id')) {

    function exertio_employer_fields_by_listing_id($classified_id = '')
    {
        $cats_value = get_post_meta($classified_id, 'cf_employer_departments', true);

        $cats_value = isset($cats_value) && $cats_value != '' ? $cats_value : array();
        $fields_data = array();
        if (!empty($cats_value) && class_exists('ACF')) {
            //$last_cat_value = end($cats_value);
            $template_group_key = apply_filters('exertio_employer_acf_get_group_key', '', $cats_value);
...
//custom fields
add_action('wp_ajax_exertio_get_employer_custom_fields', 'exertio_employer_get_custom');
add_action('wp_ajax_nopriv_exertio_get_employer_custom_fields', 'exertio_employer_get_custom');
if (!function_exists('exertio_employer_get_custom'))
{
    function exertio_employer_get_custom()
	{
        if (!empty($_POST['cat_parent']))
        {
			...
			$category_details = get_term_by('id',$parent_category, 'departments');
			if(!empty($category_details))
			{
				...
				$template_group_key = apply_filters('exertio_employer_acf_get_group_key', '',$category_id);

                if (isset($template_group_key) && $template_group_key != '' && class_exists('ACF')) {
					$fields_data = acf_get_fields($template_group_key);
					$custom_fields_html = apply_filters('exertio_employer_acf_frontend_html', '', $fields_data, 0, $col, $cus_type);
