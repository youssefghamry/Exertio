...
class Call_To_Action_One extends Widget_Base {
	...
	public function get_title() {
		return __( 'Call to action 1', 'exertio-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_query',
			[
				'label' => esc_html__( 'Call to action one Content', 'exertio-elementor' ),
			]
		);
		$this->add_control(
			'sub_title_text',
		...
		$params['side_image_positiony'] = $settings['side_image_positiony'];
			...
			echo exertio_element_call_to_action_one($params);
		}
